...
import { Prisma, City, FoodPlace, StartStopPlace } from "@prisma/client";
...
export class CityServiceBase {
  ...
  async findFoodPlaces(
    parentId: string,
    args: Prisma.FoodPlaceFindManyArgs
  ): Promise<FoodPlace[]> {
  ...
  async findStartStopPlaces(
    parentId: string,
    args: Prisma.StartStopPlaceFindManyArgs
  ): Promise<StartStopPlace[]> {
    return this.prisma.city
      .findUnique({
        where: { id: parentId },
      })
      .startStopPlaces(args);
  }
}
